...
# Init the CSV file
with open('./scrapper/nasa_dataset.csv', 'w', newline='') as csvfile:
    csvwriter = csv.writer(csvfile)
    csvwriter.writerow(["date", "absolute_magnitude_h", "estimated_diameter_min", "estimated_diameter_max"])
...
for date in api_json["near_earth_objects"]:
    for object in nr_erth_obj[date]:
        with open('./scrapper/nasa_dataset.csv', 'a', newline='') as csvfile:
            ...
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow([date, magnitude, diam_min, diam_max])
